# WhyBuddy Skill 闭环总图（改进版 v3 · 一切可验证都进台账）
...
subgraph S6["06 效果预览与交付 / Preview and Handoff"]
  ...
  EP_VIS_GEN["◆ 视觉预览·生成 / Gen Preview<br/>按模块(每需求一页)→生图提示词→模型<br/>UI 草样 · 标『预览·未验证』"]:::preview
...
## v3 这一版加了什么（相对 v2）
- **伴随层留痕进台账（◆）**：挑刺者/接地者发力后产出 `companion_log.json`，经 `check_companion.py` 校验，结果记入 `checks_ledger.json`——没留痕=没发力，可查。
- **出图变强制 gate 并进台账（◆）**：`finalize_previews.py` 作为收尾 gate，按 `spec_tree` 的每个需求(页面)逐张出图，核验 `previews/` 有真 `.png`，没出到就判不通过并记台账。
- **视觉生成由模块驱动（◆）**：`SP_TREE → EP_VIS_GEN` 按模块出图（每需求一页），不再「一份文档一张」导致重复。
- 一句话：**凡是声称做过的（校验/伴随/出图），都必须在 `checks_ledger.json` 里留下真跑的证据。台账=问责中枢。**
