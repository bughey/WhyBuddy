...
- [x] 2. Phase 2: Role Registry 与 Decision Gate
  ...
  - [x] 2.2 Create `server/routes/blueprint/brainstorm/decision-gate.ts`
  ...
  - [x] 2.3 Write property test for Decision Gate schema completeness (Property 1)
    - **Property 1: Decision Gate schema completeness**
    - For any valid LLM response, verify all required fields present and valid
    - **Validates: Requirements 1.2**

  - [x] 2.4 Write property test for Decision Gate routing correctness (Property 2)
  ...
  - [x] 2.5 Write property test for Decision Gate failure fallback (Property 3)
  ...
  - [x] 2.6 Write property test for degraded mode bias (Property 26)
  ...
  - [x] 2.7 Write unit tests for Decision Gate
...
- [x] 3. Checkpoint - Phase 1 & 2 验证
  - Ensure all tests pass, ask the user if questions arise.

- [x] 4. Phase 3: Brainstorm Orchestrator 核心
  - [x] 4.1 Create `server/routes/blueprint/brainstorm/orchestrator.ts` scaffold
  ...
  - [x] 4.2 Implement Discussion mode execution in orchestrator
  ...
  - [x] 4.3 Implement Vote mode execution in orchestrator
  ...
  - [x] 4.7 Implement session lifecycle state transitions
  ...
  - [x] 4.8 Write property test for Crew Member instantiation (Property 4)
  ...
  - [x] 4.9 Write property test for Crew Member state invariant (Property 5)
    - **Property 5: Crew Member state invariant**
    - Verify state is always exactly one of the 6 valid states
    - **Validates: Requirements 2.3**

  - [x] 4.10 Write property test for terminal state triggers synthesis (Property 6)
...
- [x] 5. Checkpoint - Orchestrator 核心验证
  - Ensure all tests pass, ask the user if questions arise.

- [x] 6. Phase 4: Tool Proxy
  - [x] 6.1 Create `server/routes/blueprint/brainstorm/tool-proxy.ts`
  ...
  - [x] 6.2 Implement permission scope validation logic
  ...
  - [x] 6.3 Implement Docker fallback and degradation handling
...
- [x] 7. Phase 5: Synthesis Engine
  - [x] 7.1 Create `server/routes/blueprint/brainstorm/synthesizer.ts`
  ...
  - [x] 7.2 Wire synthesizer into orchestrator session completion
  ...
  - [x] 7.3 Write property test for synthesis receives all crew outputs (Property 20)
    - **Property 20: Synthesis receives all crew outputs**
    - Synthesis call receives outputs of ALL completed crew members
    - **Validates: Requirements 8.1**

  - [x] 7.4 Write property test for synthesis output schema completeness (Property 21)
  ...
  - [x] 7.5 Write property test for synthesis fallback (Property 23)
  ...
  - [x] 7.6 Write unit tests for Synthesizer
...
- [x] 8. Checkpoint - Tool Proxy & Synthesis 验证
  - Ensure all tests pass, ask the user if questions arise.

- [x] 9. Phase 6: Event Bus 集成与 Socket.IO Relay
  - [x] 9.1 Register `"brainstorm"` in `DEFAULT_RELAY_FAMILIES` for Socket.IO relay
  ...
  - [x] 9.2 Implement brainstorm event emission in orchestrator
  ...
  - [x] 9.3 Implement event causal ordering guarantee
    - Ensure parent node events are emitted before child node events
    - Use sequence numbers for replay ordering
    - _Requirements: 5.6_

  - [x] 9.4 Write property test for event schema completeness (Property 12)
  ...
  - [x] 9.5 Write property test for event causal ordering (Property 13)
    - **Property 13: Event causal ordering**
    - Parent node events precede child node events within a session
    - **Validates: Requirements 5.6**

  - [x] 9.6 Write property test for degradation event emission (Property 25)
  ...
  - [x] 9.7 Write unit tests for event emission
...
- [x] 10. Phase 7: Frontend Store Slice (brainstormGraph)
  - [x] 10.1 Add `brainstormGraph` slice to `client/src/lib/blueprint-realtime-store.ts`
    - Define `BrainstormGraphSlice` state shape
    - Initialize with idle state, empty nodes/edges
    - _Requirements: 6.1_

  - [x] 10.2 Implement store event handlers for brainstorm events
  ...
  - [x] 10.5 Write property test for store node addition invariant (Property 14)
...
- [x] 11. Checkpoint - Event Bus & Store 验证
  - Ensure all tests pass, ask the user if questions arise.

- [x] 12. Phase 8: Wall Graph Renderer (BrainstormWallGraph)
  - [x] 12.1 Create `client/src/components/three/scene-fusion/BrainstormWallGraph.tsx`
  ...
  - [x] 12.2 Implement dagre layout computation
  ...
  - [x] 12.3 Implement Canvas2D node rendering
  ...
  - [x] 12.7 Wire BrainstormWallGraph to brainstormGraph store
...
- [x] 13. Phase 9: Memory Store & Replay API
  - [x] 13.1 Create `server/routes/blueprint/brainstorm/memory-store.ts`
  ...
  - [x] 13.2 Wire memory store into orchestrator session completion
  ...
  - [x] 13.3 Implement Replay API endpoint
  ...
  - [x] 13.4 Write property test for session persistence round-trip (Property 22)
  ...
  - [x] 13.5 Write unit tests for Memory Store
...
- [x] 14. Checkpoint - Wall Graph & Memory Store 验证
  - Ensure all tests pass, ask the user if questions arise.

- [x] 15. Phase 10: Diagnostics Extension
  - [x] 15.1 Extend `GET /api/blueprint/diagnostics` with brainstormOrchestrator entry
  ...
  - [x] 15.2 Write unit tests for diagnostics extension
    - Test diagnostics response includes brainstormOrchestrator entry
    - Test counter accuracy after multiple sessions
    - _Requirements: 10.6_

- [x] 16. Phase 11: Pipeline Integration (Hook into Autopilot Stages)
  - [x] 16.1 Add `brainstormOrchestrator` to `BlueprintServiceContext`
  ...
  - [x] 16.2 Integrate Decision Gate into autopilot pipeline stage driver
  ...
  - [x] 16.3 Implement graceful degradation at pipeline level
  ...
  - [x] 16.4 Emit `brainstorm.mode.selected` event when mode is chosen
    - Include mode name and participating role IDs in event payload
    - _Requirements: 3.5_

  - [x] 16.5 Write unit tests for pipeline integration
...
- [x] 17. Checkpoint - Pipeline Integration 验证
  - Ensure all tests pass, ask the user if questions arise.

- [x] 18. Phase 12: Integration Tests
  - [x] 18.1 Write integration test: Event flow end-to-end
    - Test Orchestrator → EventBus → Socket.IO relay → Store update chain
    - Verify events arrive in correct order at store
    - _Requirements: 5.1, 5.2, 5.6_

  - [x] 18.2 Write integration test: Replay API endpoint
  ...
  - [x] 18.5 Write integration test: Session persistence round-trip via API
    - Test full flow: start session → complete → persist → retrieve via API
    - Verify artifact completeness
    - _Requirements: 8.4, 9.1, 9.3_

- [x] 19. Phase 13: End-to-End Smoke Tests
  - [x] 19.1 Write smoke test: Brainstorm disabled by default
  ...
  - [x] 19.2 Write smoke test: Full brainstorm session lifecycle
