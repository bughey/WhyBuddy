...
定义 Cube Pets Office 在“任务自动驾驶”叙事下的一组统一核心对象，用于统一产品语言、界面呈现、运行时映射与后续 specs 的边界。
...
- `Replan` 由 `Risk`、低 `Confidence`、外部变化或人工决策触发
- `Confidence` 与 `Risk` 共同影响是否继续自动执行、降级执行或请求接管
