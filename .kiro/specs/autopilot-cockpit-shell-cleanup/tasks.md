...
- [ ] 1. 删除 `AutopilotWorkflowRail.case "fabric"` 分支开头的 `AutopilotSpecTreeHandoffPanel` 与 dashed 空态
...
- [ ] 2. 删除 `AutopilotWorkflowRail` return 块末尾的 `<RailMetricsBlock>`
...
- [ ] 3. 清空 `AutopilotRightRail` 非 fabric 分支内部的 label 文本
...
- [ ] 4. 清空 `AutopilotRightRail` fabric 分支顶部的 label 文本
...
- [ ] 5. 删除 `AutopilotRightRail` aside 根节点末尾的 `<RailMetricsBlock>` 与对应 import
