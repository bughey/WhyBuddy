...
export function initRAG(): InitializedRAGDeps {
  ...
  chunkRouter.register('sliding_window', () => SlidingWindowChunker.fromConfig(config.chunking.task_result));
  chunkRouter.register('syntax_aware', () => CodeChunker.fromConfig(config.chunking.code_snippet));
  chunkRouter.register('conversation_turn', () => ConversationChunker.fromConfig(config.chunking.conversation));
  chunkRouter.register('semantic_paragraph', () => DocumentChunker.fromConfig(config.chunking.document));
  chunkRouter.register('passthrough', () => PassthroughChunker.fromConfig(config.chunking.architecture_decision));
  ...
  const lifecycleManager = new LifecycleManager(vectorStore, metadataStore, hotColdManager);
