...
describe("blueprint mcp-github capability bridge 锟?e2e", () => {
  ...
  it("falls back to simulated output when the LLM call throws", async () => {
      ...
      const isAigcCall = (messages: any): boolean => {
        const text = JSON.stringify(messages);
        return /AIGC Spec Node|AIGC Spec Node 领域推理器/.test(text);
      };
...
describe("blueprint prompt-package LLM E2E", () => {
  ...
  function isPromptPackageCall(messages: any): boolean {
    const text = JSON.stringify(messages);
    return /Prompt Package 生成器|Prompt Package generator/i.test(text);
  }
